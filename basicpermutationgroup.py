...
def Order(H):
    #Termination
    NonTrivialOrbit = FindNonTrivialOrbit(H)            # Find point with non-trivial orbit
    if NonTrivialOrbit == None:                         # Check if point is found
        return 1                                        # If not --> |H| = 1

    # Multiply with order of orbit
    Orbit_a = Orbit(H, NonTrivialOrbit)                 # Create orbit of found point
    H_a = Stabilizer(H, NonTrivialOrbit)                # Create stabilizer around found point
    order = Order(H_a) * len(Orbit_a)                   # Calculate order of stabilizer
    return order


def MembershipTest(H, f):
    #base cases
    if len(H) < 2:
        return f in H
    if f in H:
    ...
    # find nontrivial alpha
    a = FindNonTrivialOrbit(H)

    #calculate orbit, traversal and stabilizer of alpha
    orbit_a, U = Orbit(H, a, returntransversal=True)
    H_a = Stabilizer(H, a)

    #beta = f[alpha]
    b = f[a]

    #search for beta in orbit of alpha
    for i in range(len(orbit_a)):
        if b == orbit_a[i]:
            u_B = U[i]
            # u_B^(-1) * f in H_a
            return MembershipTest(H_a, -u_B*f)
    return False
